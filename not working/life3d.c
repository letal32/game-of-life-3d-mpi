...
int main(int argc, char *argv[]){
        ...
        for (int i = p-1; i >= 0; i--){
            ...
            while (fgets(line, sizeof(line), file)) {
                ...
                if ((i != 0) && (x >= boundaries[2*i] && x <= boundaries[2*i+1]) || (x == mod(boundaries[2*i]-1, SIZE_CUBE)) || (x==mod(boundaries[2*i+1]+1, SIZE_CUBE))){
                    ...
                    buffer[k+2] = z;
                    k += 3;
                } else if ((i == 0) && (x >= boundaries[2*i] && x <= boundaries[2*i+1]) || (x == mod(boundaries[2*i]-1, SIZE_CUBE)) || (x==mod(boundaries[2*i+1]+1, SIZE_CUBE))){
                    buffer0[k] = x;
                    buffer0[k+1] = y;
                    buffer0[k+2] = z;
                    k += 3;
  ...
  int SIZE_MAIN_TABLE = 1134;  //The main table has more entries
  ...
  //insert_first_batch(cur_main_table, low_main_table, high_main_table, first_batch, count_batch, SIZE_MAIN_TABLE, SIZE_SIDE_TABLE, ghost_up, ghost_down);
